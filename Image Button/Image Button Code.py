...
# Create Table
mycursor.execute(
	"""CREATE TABLE IF NOT EXISTS usercredentials(username VARCHAR(255),password VARCHAR(255),userID int)""")
...
#Define Image
loginbuttonimage=Image.open("Login-Icon.png")
loginbuttonimageph=ImageTk.PhotoImage(loginbuttonimage)
...
# Buttons
loginButton = tk.Button(loginFrame, command=login,image=loginbuttonimageph)
loginButton.image=loginbuttonimageph
...
passwordRegLabel = tk.Entry(registerFrame, textvariable=inputedPassword, fg="black", font=labelFont, bg='white',
							show='*')
