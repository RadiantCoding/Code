...
# Create mysql
mycursor.execute("""CREATE TABLE IF NOT EXISTS employee(employeeID int,firstname VARCHAR(255),secondname VARCHAR(255),gender BOOLEAN,jobtype VARCHAR(255),hourlywage FLOAT, dateJoined DATE)""")
# import mysql.connector
root = tk.Tk()
...
def create():
	...
	insertSQL = """INSERT INTO employee (employeeID,firstname,secondname,gender,jobtype,hourlywage,dateJoined) VALUES(%s,%s,%s,%s,%s,%s,%s)"""
	if gender.get() =="Male":
		mycursor.execute(insertSQL,(employeeID,firstName.get(),secondName.get(),False,job.get(),wage.get(),date))
	else:
		mycursor.execute(insertSQL,(employeeID,firstName.get(),secondName.get(),True,job.get(),wage.get(),date))
	mydb.commit()
	treeviewEmployeeUpdate()
	pass
...
def treeviewEmployeeUpdate():
	...
	mycursor.execute("SELECT firstname,secondname,gender,jobtype,hourlywage,dateJoined FROM employee")
	myresults = mycursor.fetchall()
	for i in myresults:
		if i[2] == False:
			EmployView.insert("","end", text="", values=(i[0],i[1],"Male",i[3],i[4],i[5]))
		else:
			EmployView.insert("", "end", text="", values=(i[0], i[1], "Female", i[3], i[4], i[5]))
	pass
...
EmployView['columns'] = ("firstname", "secondname", "gender", "jobtype", "hourlywage", "dateJoined")
...
EmployView.heading("dateJoined", text="Date Joined", anchor="w")
EmployView.column("dateJoined", anchor="center", width=80)
...
#Not implemented with new database changes, this video is primarily focused on showing CREATE TABLE IF NOT EXISTS
...
createGenderRBM = tk.Radiobutton(createFrame, text="Male", variable=gender, value="Male", font=labelFont,
								 bg='lightblue', fg='blue')
createGenderRBM.grid(row=4, column=2)
createGenderRBF = tk.Radiobutton(createFrame, text="Female", variable=gender, value="Female", font=labelFont,
								 bg='lightblue', fg='blue')
...
# Button
retrievecButton = tk.Button(retrieveFrame, text="Retrieve", command=retrieve, font=buttonFont, bg="blue",
							fg="lightblue")
...
updateGenderRBM = tk.Radiobutton(updateFrame, text="Male", variable=gender, value="Male", font=labelFont,
								 bg='lightblue', fg='blue')
updateGenderRBM.grid(row=5, column=2)
updateGenderRBF = tk.Radiobutton(updateFrame, text="Female", variable=gender, value="Female", font=labelFont,
								 bg='lightblue', fg='blue')
