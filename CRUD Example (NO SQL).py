...
if True:
	mydb = mysql.connector.connect(
		host="localhost",
		user="root",
		# Define Your Password and Database Name
	)
...
# Create mysql
mycursor.execute(
	"""CREATE TABLE IF NOT EXISTS employee(employeeID int ,firstname VARCHAR(255),secondname VARCHAR(255),gender VARCHAR(255),jobtype VARCHAR(255),hourlywage int)""")
# import mysql.connector
root = tk.Tk()
...
def treeviewEmployeeUpdate():
	# Deletes Everything from Treeview
	Remove = EmployView.get_children()
	for child in Remove:
		EmployView.delete(child)
	# Adds Everything back into Treeview
...
EmployView.column("secondname", anchor="center", width=90)
EmployView.heading("gender", text="Gender", anchor="w")
EmployView.column("gender", anchor="center", width=90)
...
retrieveButton = tk.Button(treeviewFrame, text="Retrieve", font=buttonFont, bg='blue', fg='lightblue',
						   command=raiseRetrieve)
